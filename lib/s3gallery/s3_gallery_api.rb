...
class S3GalleryApi
  ...
  def create_thumbnail(picdata, tn)
    begin
      ...
      if tn[:style] == :resize_to_fill
        ...
        thumbnail_image = picdata.resize_to_fit tn[:width], tn[:height]
      end
      thumbnail_image
    rescue Exception => e
  ...
  def wrap_generate_thumbnail
    lambda { |s3img|
      ...
      puts "Retrieving #{original_key}:#{original_etag}:#{original_guid}"

      @thumbnail_size.each do |tn|
        thumbnail_filename = "#{@thumbnail_dir}/#{tn[:width]}x#{tn[:height]}/#{s3img.guid}.jpg"
        next if safe_s3_exists? thumbnail_filename  # Already generated
        puts "Generating Thumbnail #{thumbnail_filename}."
        thumbnail_image = create_thumbnail s3img.magick_image, tn
        next if thumbnail_image.nil?
        # first do an empty save due to the fact that we will have to "download" it again in order to change the
        # metadata.
        response = AWS::S3::S3Object.store thumbnail_filename, '', @gallery_bucket
        if response.error?
